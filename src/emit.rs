...
/// 3. CRT0   – stack init, call to main, HLT (entry point `_start` now at 0x100).
/// 4. All assembly lines produced by the code generator / peephole optimizer.
/// 5. Referenced runtime library routines (auto-detected from CALL/JMP).
...
fn build_full_asm(code_lines: &[String]) -> Vec<String> {
    ...
    // Inline main function if present in code_lines.
    let (code_lines, inlined_main_body) = extract_main_body(code_lines);
    if !inlined_main_body.is_empty() {
        if let Some(call_idx) = lines
            .iter()
            .position(|l| l.trim().eq_ignore_ascii_case("CALL main"))
        {
            lines.splice(call_idx..call_idx + 1, inlined_main_body.clone());

            // If main is inlined, the CRT0 HLT after CALL main becomes redundant.
            let post_main_idx = call_idx + inlined_main_body.len();
            if post_main_idx < lines.len() {
                let mnemonic = lines[post_main_idx]
                    .split(';').next().unwrap_or("")
                    .split_whitespace().next().unwrap_or("")
                    .to_ascii_uppercase();
                if mnemonic == "HLT" {
                    lines.remove(post_main_idx);
                }
            }
        }
    }

    // Code from the code generator (remaining functions)
    ...
    let mut runtime_scan_lines = code_lines.clone();
    runtime_scan_lines.extend(inlined_main_body.clone());
    let rt = runtime::collect_runtime(&runtime_scan_lines);
    if !rt.is_empty() {
...
fn extract_main_body(code_lines: &[String]) -> (Vec<String>, Vec<String>) {
    let mut rest = code_lines.to_owned();

    let main_start = match rest.iter().position(|l| l.trim() == "main:") {
        Some(i) => i,
    ...
    let mut main_body = rest[main_start + 1..main_end].to_vec();
    for line in main_body.iter_mut() {
        if line.trim().eq_ignore_ascii_case("RET") {
            *line = "\tHLT".to_string();
        }
    }
...
mod tests {
    ...
    fn order_is_correct() {
        ...
        let start_pos = out.find("_start:").unwrap();
        let nop_pos = out.find("\tNOP").unwrap();
        let rt_pos = out.find("; --- runtime library ---").unwrap();

        assert!(org_pos < start_pos);
        assert!(start_pos < nop_pos);
